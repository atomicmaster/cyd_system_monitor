...
V1 adds a Settings-configurable 180-degree landscape display flip so the physical unit can be mounted with its USB connector on either side without requiring a new screen layout: the existing 320x240 landscape geometry is unchanged, only the MADCTL mirror bits and the touch calibration transform's coordinate mapping invert together. Reusing a stale calibration transform across an orientation change would map taps to the wrong screen location, so this setting change follows the same invalidation rule already required for a hardware-profile or calibration-schema change: it forces recalibration rather than silently reprojecting old touch points, using the calibration record's existing `orientation` identity field (checksummed alongside profile id and schema version since the MVP) to detect and reject a mismatched record. The MVP's E32R28T profile requirements fix a single landscape orientation with no runtime toggle; this is deferred here rather than added mid-MVP because it is a genuine product-scope change to that fixed requirement, not a bug fix.
...
MicroSD currently shares the ESP32's second hardware SPI controller while touch uses bit-banged software SPI (the board wires three separate SPI buses against only two hardware controllers; see [F08](tickets/F08.md) and the E32R28T profile's `development/feasibility.md`). That arrangement was confirmed workable under combined radio/UI/serial load, but MicroSD write throughput was never a load-bearing requirement until Survey Sessions start writing WiGLE artifacts to it. Revisit whether touch and MicroSD should swap arrangements before implementing Survey Sessions, not before.
