...
M1a validates the combined radio/display/touch/USB workload and establishes memory and flash budgets before the full Host application and UI work. Display, touch, and MicroSD have separate signal buses on this board, while ESP32 has two general-purpose SPI controllers, so exactly one peripheral must be software-driven; F08 chose touch (display keeps hardware SPI2 undisturbed, MicroSD keeps hardware SPI3) and confirmed it under repeated combined radio/UI/serial load with no observed loss or degradation. An alternative (hardware-SPI touch, bit-banged MicroSD) was evaluated and deliberately deferred to before V2's SD-backed WiGLE Survey Artifacts, the first feature that needs MicroSD write bandwidth -- see `development/feasibility.md`'s "Required next physical evidence" section. Espressif also identifies performance instability for concurrent WiFi-sniffer/BLE operation, so scheduled windows require measured coverage and switching limits. See [SPI controller documentation](https://docs.espressif.com/projects/esp-idf/en/release-v4.4/esp32/api-reference/peripherals/spi_master.html) and [RF coexistence documentation](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-guides/coexist.html).
